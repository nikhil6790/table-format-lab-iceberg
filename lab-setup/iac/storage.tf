...
resource "null_resource" "create_mnbs_post_startup_bash" {
  provisioner "local-exec" {
    #For BSD based systems (MacOS)  use below command
    command = "cp ./templates/mnbs-exec-post-startup-template.sh ../notebooks/mnbs-exec-post-startup.sh && sed -i '' s/YOUR_PROJECT_NBR/${local.project_nbr}/g ../notebooks/mnbs-exec-post-startup.sh"
  }

    #For GNU based systems (LINUX) use below command
    #command = "cp ./templates/mnbs-exec-post-startup-template.sh ../notebooks/mnbs-exec-post-startup.sh && sed -i  s/YOUR_PROJECT_NBR/${local.project_nbr}/g ../notebooks/mnbs-exec-post-startup.sh"
...
resource "google_storage_bucket_object" "upload_notebooks_to_code_bucket" {
...
resource "time_sleep" "sleep_after_bucket_uploads" {
  ...
  depends_on = [
    ...
    google_storage_bucket_object.upload_notebooks_to_code_bucket,
